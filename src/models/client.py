...
class Client(object):
    ...
    def __init__(self, cid, group, train_data, test_data, batch_size, worker):
        ...
        # print(self.train_dataloader.shape)
    ...
    def local_train(self,latest_model=None, num_epoch=None,**kwargs):
        ...
        local_solution, worker_stats = self.worker.local_train(self.train_dataloader, num_epoch,latest_model=latest_model, **kwargs)
        end_time = time.time()
        bytes_r = self.worker.model_bytes
        ...
        return (len(self.train_data),self.cid, local_solution), stats

    def local_test(self, use_eval_data=True):
        """Test current model on local eval cifar-10-batches-py
